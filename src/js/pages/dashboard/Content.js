...
class DashboardContent extends Component {
  ...
  componentDidMount() {
    ...
    var simulation = d3.forceSimulation().force("link", d3.forceLink().id(function(d) {
      return d.id;
    }).strength(0.3)).force("charge", d3.forceManyBody()).force("center", d3.forceCenter(960 / 2, 540 / 2));
    ...
    graph.data({
      nodes: _data.nodes,
      edges: _data.edges.map(function(edge, i) {
        edge.id = 'edge' + i;
        return Object.assign({}, edge);
      })
    });
    simulation.nodes(_data.nodes).on("tick", ticked);
    simulation.force("link").links(_data.edges);
